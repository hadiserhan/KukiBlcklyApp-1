...
async function webUSBOpenConnection() {
    ...
    try {
      ...
      if(e.message=="Failed to execute 'claimInterface' on 'USBDevice': Unable to claim interface."){
        ...
        alert("Edison is connected to a different tab, please referesh that tab to break the connection then retry");
      }else {
...
async function pushUIDtoBox(){
    // const uidNumber = await getEdisonV3UID();
