...
async function prepareEdisonAndSendFirmware() {
  ...
  if (version[0] == EV3_VARIANT_CODE_BOOTLOADER) {
    ...
    if (version[2].startsWith("ÿ")||!(version[2].startsWith("v"))) {
      ...
      console.log("Edison only has bootloader mode");
      bootOnly = true;
    }
  ...
  if ( !version[1].startsWith(updateBootloaderVersion)) {
    ...
    getFileFromServer(urlUpdateBootloader).then(async fileContent => {
      ...
      if (fileContent) {
        ...
        if (await programEV3Firmware(bootloader, true)) {
          ...
          // maybe add a pause and some feedback to user...
          fuFeebackCardAppend('Please click <strong>Reconnect Edison</strong>.', 'prompt');

          // Add button to UI with link to recall this function - prepareEdisonAndSendFirmware()
    ...
    if (bootOnly) {
      ...
      if ( version[2].startsWith(updateFirmwareVersion)) {
        ...
        fuFeebackCardAppend('No update required: the firmware on your Edison is up to date.', 'success');
        //await webUSBCommandReset(1);
