...
var myStartScale = ((availWidth - 1000) / 300) * 0.8;
...
let startCode = '<xml xmlns="http://www.w3.org/1999/xhtml"><block type="event_whenflagclicked" maxInstances="1" id="}^Xg:jW-h7O?ksLFb:ke" x="600" y="270" deletable="false"></block></xml>';
...
function change(event) {
  ...
  if (
    ...
    (event.type === Blockly.Events.CHANGE && event.element === 'field' && event.name === 'NAME')
  ) {
    ...
    if (block && (
      ...
      // Re-render toolbox to show updated function call blocks
    ...
    if(event.type == "ui")
    {
      ...
      if (block != null && block.type == "event_Start") {
      ...
      if(id != null && workspace.getBlockById(id) != null)
      {
        ...
        if(block.childBlocks_ != null && block.type == "event_Start")
        {
            StartBlockId = id;
        }
      }
...
function ottoStartclickedID(xml) {
    ...
    var startIndex = xml.indexOf('<block type="event_Start" id="');
